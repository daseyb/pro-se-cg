...
\begin{figure}[htb!]
  ...
  \begin{tikzpicture}
   \begin{polaraxis}[xmin=0,xmax=180,ymax=1,
	...
	\addplot+[mark=f,domain=0:180,samples=600] 
		{ (sin(2*x)*cos(2*x) + cos(0.3*x + 1) - sin(30*x)*0.05 + cos(100*x)*0.02)/1.75 }; 
    \addplot+[mark=g,domain=0:180,samples=600] 
		{(sin(2*x)*cos(2*x) + cos(0.3*x + 1))/1.75 };
    \addplot+[dashed, polar comb,domain=0:180, samples=6,black,opacity=0.75,mark=x]
	    {(sin(2*x)*cos(2*x) + cos(0.3*x + 1) - sin(30*x)*0.05 + cos(100*x)*0.02)/1.75  };
	\addplot+[dashed, polar comb,domain=10:45, samples=4,black,opacity=0.75,mark=x]
	    {(sin(2*x)*cos(2*x) + cos(0.3*x + 1) - sin(30*x)*0.05 + cos(100*x)*0.02)/1.75  };
	\addplot+[dashed, polar comb,domain=15:40, samples=3,black,opacity=0.75,mark=x]
	    {(sin(2*x)*cos(2*x) + cos(0.3*x + 1) - sin(30*x)*0.05 + cos(100*x)*0.02)/1.75  };
	\addplot+[dashed, polar comb,domain=20:35, samples=2,black,opacity=0.75,mark=x]
	    {(sin(2*x)*cos(2*x) + cos(0.3*x + 1) - sin(30*x)*0.05 + cos(100*x)*0.02)/1.75  };
  ...
  \end{tikzpicture}\par
  \end{centering}
  \caption{Approximating a complicated function $f$ by a simpler function $g$ and sampling $f$ based on the probability distribution of $g$. Note how we are more likely to sample the $f$ in directions where it has a large value.}
...
\begin{equation}
\frac{1}{N} \sum_{i=1}^{N} f(X_i), X \sim U \rightarrow \frac{1}{N} \sum_{i=1}^{N} \frac{f(X_i)}{p(X_i)}, X \sim p
\end{equation}
...
\begin{figure}[htb!]
  ...
  \end{tikzpicture}\par
  \end{centering}
  \caption{Distribution we want to sample to account for Lambert's cosine law}
