...
\begin{split}
V(\frac{f(x)}{p(x)}) & = E((\frac{f(x)}{p(x)})^2) - E(\frac{f(x)}{p(x)})^2 \\ & \approx \frac{1}{N}\sum_{i=1}^{N}{(\frac{f(X_i)}{p(X_i)})^2} - (\frac{1}{N}\sum_{i=1}^{N}{\frac{f(X_i)}{p(X_i)}})^2 
 = \frac{1}{N}\sum_{i=1}^{N}{(\frac{f(X_i)}{p(X_i)})^2} - \frac{1}{N^2}(\sum_{i=1}^{N}{\frac{f(X_i)}{p(X_i)}})^2 \\
& \underset{\mathrm{f(x) \propto p(x)}}{=} \frac{1}{N}\sum_{i=1}^{N}{c^2} - \frac{1}{N^2}(\sum_{i=1}^{N}{c})^2
 = \frac{1}{N}N{c^2} - \frac{1}{N^2}(N{c})^2
 = c^2 - c^2 = 0 
\end{split}
\end{equation}

As such, if $f$ is proportional to $p$ the variance of the resulting Monte Carlo integrator is $0$. It is easy to show that this is a continuous relation, i.e. the closer $f$ and $p$ get to being proportional, the lower the variance.
...
\begin{figure}[htb!]
  ...
  \end{tikzpicture}\par
  \end{centering}
  \caption{Sampling a distribution $p$ which only accounts for Lambert's cosine law}
