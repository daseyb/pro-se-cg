...
const size_t MAX_COMPONENTS = 2048;
...
public:
  ...
  */
  ...
  /**
  * Invalidate Entity handle, disassociating it from an EntityManager and
  * invalidating its ID.
  *
  ...
  */
  ...
  template <typename C> ComponentHandle<C> assign_from_copy(const C &component);
...
private:
  ...
  ComponentHandle(EM *manager, Entity::Id id) : manager_(manager), id_(id) {}
...
* Components SHOULD provide convenience constructors for initializing on
* assignment to an Entity::Id.
*
...
public:
  ...
  protected:
    ...
    ViewIterator(EntityManager *manager, const ComponentMask mask,
                 uint32_t index)
        : manager_(manager), mask_(mask), i_(index),
          capacity_(manager_->capacity()), free_cursor_(~0UL) {
      if (All) {
        std::sort(manager_->free_list_.begin(), manager_->free_list_.end());
        free_cursor_ = 0;
      }
    ...
    inline bool predicate() {
      return (All && valid_entity()) ||
             (manager_->entity_component_mask_[i_] & mask_) == mask_;
    }
  ...
  template <bool All> class BaseView {
  public:
  ...
  private:
    ...
    explicit BaseView(EntityManager *manager) : manager_(manager) {
      mask_.set();
    }
  ...
  private:
    ...
    TypedView(EntityManager *manager, ComponentMask mask)
  ...
  public:
    ...
    Iterator end() {
      return Iterator(manager_, mask_,
                      static_cast<uint32_t>(manager_->capacity()), unpacker_);
    }
    const Iterator begin() const {
      return Iterator(manager_, mask_, 0, unpacker_);
    }
    const Iterator end() const {
      return Iterator(manager_, mask_,
                      static_cast<uint32_t>(manager_->capacity()), unpacker_);
  ...
  private:
    ...
    UnpackingView(EntityManager *manager, ComponentMask mask,
  ...
  */
  bool valid(Entity::Id id) const {
    return id.index() < entity_version_.size() &&
  ...
  */
  template <typename C> void remove(Entity::Id id) {
    assert_valid(id);
    const BaseComponent::Family family = component_family<C>();
  ...
  */
  template <typename C> bool has_component(Entity::Id id) const {
    assert_valid(id);
  ...
  */
  DebugView entities_for_debugging() { return DebugView(this); }

  template <typename C> void unpack(Entity::Id id, ComponentHandle<C> &a) {
    assert_valid(id);
    a = component<C>(id);
  }
  ...
  */
  template <typename A, typename... Args>
  void unpack(Entity::Id id, ComponentHandle<A> &a,
  ...
  */
  ...
  template <typename C> static BaseComponent::Family component_family() {
...
private:
  ...
  inline void assert_valid(Entity::Id id) const {
    assert(id.index() < entity_component_mask_.size() &&
           "Entity::Id ID outside entity vector range");
    assert(entity_version_[id.index()] == id.version() &&
           "Attempt to access Entity via a stale Entity::Id");
  }

  template <typename C> C *get_component_ptr(Entity::Id id) {
    assert(valid(id));
  ...
  template <typename C> const C *get_component_ptr(Entity::Id id) const {
    assert_valid(id);
    BasePool *pool = component_pools_[component_family<C>()];
    assert(pool);
  ...
  template <typename C> ComponentMask component_mask() {
    ComponentMask mask;
  ...
  ComponentMask component_mask(const ComponentHandle<C1> &c1,
                               const ComponentHandle<Components> &... args) {
  ...
  template <typename C> Pool<C> *accomodate_component() {
...
template <typename C> BaseComponent::Family Component<C>::family() {
...
template <typename C, typename> ComponentHandle<C> Entity::component() {
  assert(valid());
...
Entity::components() const {
  assert(valid());
  return const_cast<const EntityManager *>(manager_)
      ->components<const Components...>(id_);
}
...
inline bool Entity::valid() const { return manager_ && manager_->valid(id_); }
...
inline bool ComponentHandle<C, EM>::valid() const {
  return manager_ && manager_->valid(id_) &&
         manager_->template has_component<C>(id_);
}
